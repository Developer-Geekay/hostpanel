...
const Databases = lazy(() => import('./features/databases/Databases'));
const Ftp       = lazy(() => import('./features/ftp/Ftp'));
const Domains   = lazy(() => import('./features/domains/Domains'));
const Redirects = lazy(() => import('./features/redirects/Redirects'));
...
function AdminRoute({ children }: { children: React.ReactNode }) {
  ...
  if (!user) return <Navigate to="/login" replace />;
  if (!isAdmin) return <Navigate to="/app/files" replace />;
  return <>{children}</>;
}
  ...
  return (
    <Routes>
      ...
      <Route path="/app" element={<ProtectedRoute><Shell /></ProtectedRoute>}>
        ...
        <Route path="files"     element={<S><Files /></S>} />
        <Route path="databases" element={<S><Databases /></S>} />
        <Route path="redirects" element={<S><Redirects /></S>} />
        <Route path="ftp"       element={<S><Ftp /></S>} />
        <Route path="domains"   element={<S><Domains /></S>} />
        {/* pkg/:slug — explicit shell for packages with frontend/main.js */}
        <Route path="pkg/:slug" element={<PackageShell />} />
        {/* :slug catch-all — plugin nav items route to /app/<slug> directly */}
