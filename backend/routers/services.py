"""
System Services & Logs API Router

Exposes endpoints for managing systemd services and fetching live daemon logs.

Path Prefix: `/cpanelapi/services`
Access Control: Admin required.

Capabilities:
- Cross-level checks: Queries both standard system-wide and user-level (`--user`) systemd services.
- Multi-action controls: Supports starting, stopping, restarting, and reloading system services.
- Journal Integration: Executes `journalctl` to safely return rotating daemon logs up to 1000 lines.

Endpoints:
- `GET `: Returns a list of built-in (API, DNS) and dynamically registered services with their current statuses (running, stopped, failed).
- `POST /{name}/start`: Starts a specific systemd service.
- `POST /{name}/stop`: Stops a specific systemd service.
- `POST /{name}/restart`: Restarts a specific systemd service.
- `POST /{name}/reload`: Reloads configuration for compatible services.
- `GET /{name}/logs`: Streams ISO-formatted systemd journal logs.
"""
import logging
import os
import subprocess
